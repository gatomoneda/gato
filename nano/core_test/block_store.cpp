...
TEST (unchecked, simple)
{
	...
	unchecked.put (block1->previous (), block1);
	unchecked.flush ();
	auto transaction = store->tx_begin_write ();
...
TEST (unchecked, multiple)
{
	...
	ASSERT_TRUE (block2.empty ());
	unchecked.put (block1->previous (), block1);
	unchecked.put (block1->source (), block1);
	unchecked.flush ();
...
TEST (rocksdb_block_store, tombstone_count)
{
	if (nano::rocksdb_config::using_rocksdb_in_tests ())
	{
		...
		unchecked.del (store->tx_begin_write (), nano::unchecked_key (block1->previous (), block1->hash ()));
